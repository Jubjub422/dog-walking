...
const cityArray = database.walkers.map(walker => {
    let cityId = walker.id
    let cityObject = {
        "id": cityId,
        "city": walker.city
    }
...
// let cityId = database.walkers.map(walker =>{
//     if (walker.id === parseInt(cityArray.id)) {
        
//     }
// })
// console.log(database.walkers)

database.walkers.forEach(walker => {
    walker['cityId'] = null
    for (const city of cityArray)
    if (city.id === walker.id) {
